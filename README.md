...
本项目依赖 GCC 编译器,编译环境为课程虚拟机VMware Workstation Pro17,操作系统为Ubuntu24.04。由于使用了 Linux 特有的清屏指令 `system("clear")`，建议在**Linux**环境下运行。Windows 用户需将代码中的 `system("clear")` 修改为 `system("cls")` 并确保终端支持 UTF-8。
...
(*注意：请确保你的目录下包含相应的头文件*:**myhead.h evaluate.h forbidden.h**，源代码默认头文件与源代码放置在同一文件夹中，如果头文件要放置在其他文件夹中需要修改源代码中对头文件的引用，且在编译时也需要加上额外指令)
